...
miRNAcomb$prepregObese = NA
miRNAcomb$prepregObese[miRNAcomb$mbmi_prepreg >= 30 & !is.na(miRNAcomb$mbmi_prepreg)] = 1
miRNAcomb$prepregObese[miRNAcomb$mbmi_prepreg < 30 & !is.na(miRNAcomb$mbmi_prepreg)] = 0
...
# path = file.path()
write.csv(miRNAcomb, paste0(path, '/miRNA_normAllAfter35replace_pheno_gt80perc_asthma.csv'))
```

## Cleaning pheno data
```{r, eval = F}
# miRNAcomb = read.csv(paste0(path, '/miRNA_normAllAfter35replace_pheno_gt80perc_asthma.csv'))[,-1]
...
miRNAcomb$prepregObese = as.factor(miRNAcomb$prepregObese)
...
dfRlmUnadj = function(dfMIRNA, var, NmiRNAs){
	...
	for (i in 1:NmiRNAs){
		set.seed(1234)
		rlm.fit = rlm(dfMIRNA[,i] ~ dfMIRNA[[var]], method = 'M', maxit = 60)
...
# Function for analyses adjusted for sex, race, education, week of breast milk collection, and maternal obesity
dfRlmAdjChageOb = function(dfMIRNA, var, NmiRNAs){
	dfMod = data.frame(matrix(ncol = 11, nrow = NmiRNAs))
	colnames(dfMod) = c('miRNA', 'B_preg', 'CI_L_preg', 'CI_U_preg', 'p_preg', 'p_FDR_preg', 'B_inact', 'CI_L_inact', 'CI_U_inact', 'p_inact', 'p_FDR_inact')
	dfMod$miRNA = colnames(dfMIRNA[c(1:NmiRNAs)])
	for (i in 1:NmiRNAs){
		set.seed(1234)
		rlm.fit = rlm(dfMIRNA[,i] ~ dfMIRNA[[var]] + dfMIRNA[['childsex']] + dfMIRNA[['momrace3']] + dfMIRNA[['edu2']] + dfMIRNA[['wk_breastmilk']] + dfMIRNA[['prepregObese']], method = 'M', maxit = 60)
		test = coeftest(rlm.fit, vcovHC(rlm.fit, type="HC0"))
		dfMod[i,2] = test[3,1]
		dfMod[i,3] = confint(test)[3,1]
		dfMod[i,4] = confint(test)[3,2]
...
# correcting microRNA names
miRBase = read.csv(paste0(path, '/miRBaseIDs.csv'), colClasses = c(rep("character",4)))
...
# saving results
write.csv(asthUnadj, file = past0(path, '/asthmaUnadj.csv'))
```
...
# saving results
write.csv(asthAdj, file = paste0(path, '/asthmaAdj.csv'))
```

### Adjusted for sex, race, education, week of breast milk collection, and pre-pregnancy obesity
```{r, eval = F}
asthAdjOb = dfRlmAdjChageOb(miRNAcomb[,-c(1:3)], 'masth3', 130)

# Active during pregnancy vs. never
table(asthAdjOb$p_preg < 0.05 & abs(asthAdjOb$B_preg) > 0.2)
# FALSE  TRUE 
  # 123     7 

table(asthAdjOb$p_FDR_preg < 0.2)
# FALSE 
  # 130 

table(asthAdjOb$p_FDR_preg < 0.1)
# FALSE 
# 130 
 
table(asthAdjOb$p_FDR_preg < 0.05)
# FALSE 
# 130 

# Inactive vs. never
table(asthAdjOb$p_inact < 0.05 & abs(asthAdjOb$B_inact) > 0.2)
# FALSE  TRUE 
  # 99    29 

table(asthAdjOb$p_FDR_inact < 0.2)
# FALSE  TRUE 
  # 101    28 
  
table(asthAdjOb$p_FDR_inact < 0.1)
# FALSE  TRUE 
  # 111    25
 
table(asthAdjOb$p_FDR_inact < 0.05)
# FALSE  TRUE 
  # 120    8 

# correcting microRNA names
asthAdjOb = merge(asthAdjOb, miRBase[,c(5,3, 4)], by = 'miRNA')

sigAsthAdj_preg_ob = asthAdjOb[asthAdjOb$p_preg <0.05, ]
sigAsthAdj_preg_ob = sigAsthAdj_preg_ob[order(sigAsthAdj_preg_ob$p_preg),]
sigAsthAdj_inact_ob = asthAdjOb[asthAdjOb$p_inact <0.05, ]
sigAsthAdj_inact_ob = sigAsthAdj_inact_ob[order(sigAsthAdj_inact_ob$p_inact),]

# saving results
write.csv(asthAdjOb, file = paste0(path, '/asthmaAdj_ob.csv'))
```
...
# quartz.save(file = paste0(path, '/rainPlotAsthmaPreg.pdf)', type = 'pdf', dpi = 300)
# 
```


```{r, echo = F, out.width = '100%', echo = F}
knitr::include_graphics(paste0(path, "/rainPlotAsthmaPreg.png"))
```
...
asthVol 
# quartz.save(file = paste0(path, '/volplotAsthmaPreg.pdf'), type = 'pdf', dpi = 300)
# quartz.save(file = paste0(path, '/volplotAsthmaPreg.png'), type = 'png', dpi = 300)
```

```{r, echo = F, out.width = '50%', echo = F}
knitr::include_graphics(paste0(path, "/volplotAsthmaPreg.png"))
```
...
# quartz.save(file = paste0(path, '/asthSigCorPlot.pdf'), type = 'pdf', dpi = 300)
# quartz.save(file = paste0(path, '/asthSigCorPlot.png'), type = 'png', dpi = 300)
```

```{r, echo = F, out.width = '50%', echo = F}
knitr::include_graphics(paste0(path, '/asthSigCorPlot.png'))
```
...
```{r, warning=FALSE,message=FALSE, eval = F}
# kegg plot
kegg = read.csv(paste0(path, '/diana_asthma_microRNAs_pathwayUnion.csv'))
...
keggmiRNAs = read.csv(paste0(path, '/diana_asthma_microRNAs_pathwayUnion_microRNAs.csv'))
...
# quartz.save(file = paste0(path, '/Fig3.pdf'), type = 'pdf', dpi = 300)
# quartz.save(file = paste0(path, '/Fig3.png'), type = 'png', dpi = 300)
```

```{r, echo = F, out.width = '100%', echo = F}
knitr::include_graphics(paste0(path, '/asthKeggPlot.png'))
```
...
# quartz.save(file = paste0(path, '/volplotAsthmaInact.png'), type = 'png', dpi = 300)

```

```{r, echo = F, out.width = '50%', echo = F}
knitr::include_graphics(paste0(path, '/volplotAsthmaInact.png'))
```
...
# saving results
write.csv(atopUnadj, file = paste0(path, '/atopyUnadj.csv'))
```
...
# saving results
write.csv(atopAdj, file = paste0(path, '/atopaAdj.csv'))
```

### Adjusted for sex, race, education, week of breast milk collection, and pre-pregnancy obesity
```{r, eval = F}
atopAdjOb = dfRlmAdjChageOb(miRNAcomb[,-c(1:3)], 'matop3', 130)

# Active during pregnancy vs. never
table(atopAdjOb$p_preg < 0.05 & abs(atopAdjOb$B_preg) > 0.2)
# FALSE  TRUE 
  # 129     1

table(atopAdjOb$p_FDR_preg < 0.2)
# FALSE 
  # 130 

table(atopAdjOb$p_FDR_preg < 0.1)
# FALSE 
# 130 
 
table(atopAdjOb$p_FDR_preg < 0.05)
# FALSE 
# 130 

# Inactive vs. never
table(atopAdjOb$p_inact < 0.05 & abs(atopAdjOb$B_inact) > 0.2)
# FALSE  TRUE 
  # 125    5 

table(atopAdjOb$p_FDR_inact < 0.2)
# FALSE 
# 130 
  
table(atopAdjOb$p_FDR_inact < 0.1)
# FALSE 
# 130 
 
table(atopAdjOb$p_FDR_inact < 0.05)
# FALSE 
# 130 

# correcting microRNA names
atopAdjOb = merge(atopAdjOb, miRBase[,c(5,3, 4)], by = 'miRNA')

sigAtopAdj_preg_ob = atopAdjOb[atopAdjOb$p_preg <0.05, ]
sigAtopAdj_preg_ob = sigAtopAdj_preg_ob[order(sigAtopAdj_preg_ob$p_preg),]
sigAtopAdj_inact_ob = atopAdjOb[atopAdjOb$p_inact <0.05, ]
sigAtopAdj_inact_ob = sigAtopAdj_inact_ob[order(sigAtopAdj_inact_ob$p_inact),]

# saving results
write.csv(atopAdjOb, file = paste0(path, '/atopAdj_ob.csv'))
```
...
# quartz.save(file = paste0(path, '/rainPlotAtopyPreg.pdf'), type = 'pdf', dpi = 300)
# quartz.save(file = paste0(path, '/rainPlotAtopyPreg.png'), type = 'png', dpi = 300)

```

```{r, echo = F, out.width = '50%', echo = F}
knitr::include_graphics(paste0(path, '/rainPlotAtopyPreg.png'))
```
...
# quartz.save(file = paste0(path, '/volplotAtopPreg.pdf'), type = 'pdf', dpi = 300)
# quartz.save(file = paste0(path, '/volplotAtopPreg.png'), type = 'png', dpi = 300)
...
# quartz.save(file = paste0(path, '/volplotAsthmaAtopPreg.pdf'), type = 'pdf', dpi = 300)
```

```{r, echo = F, out.width = '50%', echo = F}
knitr::include_graphics(paste0(path, '/volplotAtopPreg.png'))
```
...
ggplot(atopAdj, aes(x = B_inact, y = logp_inact)) + geom_point(size = 1.3, alpha = atopAdj$sig_inact, shape = 16) + geom_hline(yintercept = 0) + 
 ...
 # quartz.save(file = paste0(path, '/volplotAtopInact.png'), type = 'png', dpi = 300)
```

```{r, echo = F, out.width = '50%', echo = F}
knitr::include_graphics(paste0(path, '/volplotAtopInact.png'))
```
...
# saving results
write.csv(asthAdj_lt12, file = paste0(path, '/asthmaAdj_lt12.csv'))
```
...
# saving results
write.csv(atopAdj_lt12, file = paste0(path, '/atopyAdj_lt12.csv'))
```
